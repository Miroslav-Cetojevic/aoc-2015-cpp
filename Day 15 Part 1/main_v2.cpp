...
using IntArray = std::array<intmax, N>;
...
auto parse(std::string_view input) {
  ...
  for(const auto& line : split_input(input)) {
    ...
    for(const auto i : boost::irange(N)) {
      ingredient[i] = to_int_suffix(words[(i+1)*2]);
    }
  }
...
auto get_max_score(const Ingredients& ingredients) {
  ...
  for(auto fr = min_spoons; fr <= (max_spoons - 3); ++fr) {

    for(auto ca = min_spoons; ca <= (max_spoons - fr - 2); ++ca) {

      for(auto bu = min_spoons; bu <= (max_spoons - fr - ca - 1); ++bu) {
        ...
        const auto new_score = std::accumulate(properties.begin(), properties.end(), intmax{1}, [] (auto acc, auto value) {
