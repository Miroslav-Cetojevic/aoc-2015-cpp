...
// std::map is used for std::includes, which requires sorted containers
using Aunt = std::map<std::string_view, uintmax>;
...
auto parse(std::string_view input) {
  ...
  for(auto line : split_input(input)) {
    ...
    const auto pos = line.find(':'); // ignore the aunt
...
auto solution(std::string_view input) {
  ...
  const auto result = std::find_if(aunts.begin(), aunts.end(), [&sue] (const auto& aunt) {
    return std::includes(sue.begin(), sue.end(), aunt.begin(), aunt.end());
  });
